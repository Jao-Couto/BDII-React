...
export default function Atendimentos(props) {
    ...
    useEffect(() => {
        if (props.colunas === "atendente") {
            ...
            atendimentosService.listarAtendimentosAtendente()
                .then((response) => {
                    let dados = response.data;
                    dados.map((elem, ind) => {
                        let data = new Date(elem['data_hora'])
                        let dataFormat = data.getDate() + "/" + (data.getMonth() + 1) + "/" + data.getFullYear() + " " + data.getHours() + ":" + data.getMinutes() + ":" + data.getSeconds();
        ...
        if (props.colunas === "medico") {
            ...
            atendimentosService.listarAtendimentosMedico()
                .then((response) => {
                    let dados = response.data;
                    dados.map((elem, ind) => {
                        let data = new Date(elem['data_hora'])
